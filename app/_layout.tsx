...
import { openDatabase } from "expo-sqlite"; // Imports SQL
...
const db = openDatabase(
  ...
  (error) => {
    console.error(error);
  } // Error callback (logs error to console)
);
...
const deleteFot = async (Id) => {
  return new Promise((resolve, reject) => {
    // Returns a promise which will be resolved after deletion
    // Use of a promise allows for the function to be used asynchronously
    db.transaction(
      (tx) => {
        // Execute the SQL query to delete the footage entry with the given 'id'.
        tx.executeSql(
          "delete from footage where id = ?", // SQL query to delete row with matching ID
          [Id]
        ); // Replaces placeholder in query with ID
      },
  ...
  return new Promise((resolve, reject) => {
    ...
    db.transaction((tx) => {
      ...
      tx.executeSql(
        ...
        (_, result) => {
          // Callback for a successful query execution
          resolve(result); // Resolve promise with the result
        },

        (_, error) => {
          // Callback for query execution failure
